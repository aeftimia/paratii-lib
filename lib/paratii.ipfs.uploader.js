...
class ParatiiIpfsUploader extends EventEmitter {
  ...
  async upload (files, options) {
    ...
    let ipfsInstance = await this._ipfs.getIPFSInstance()
    this._ipfs.start(() => {
      ...
      pull(
        ...
        pull.asyncMap((file, cb) => pull(
          ...
          ipfsInstance.files.addPullStream({chunkerOptions: {maxChunkSize: this._chunkSize}}), // default size 262144
  ...
  async addDirectory (dirPath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const addStream = ipfsInstance.files.addReadableStream()
  ...
  transcode (fileHash, options) {
    ...
    let ipfsInstance = this._ipfs.getIPFSInstance()
    ipfsInstance.swarm.connect(opts.transcoder, (err, success) => {
      if (err) return opts.onError(err)
      ipfsInstance.swarm.peers((err, peers) => {
  ...
  signalTranscoder (file, callback) {
    ...
    let ipfsInstance = this._ipfs.getIPFSInstance()
    ipfsInstance.swarm.connect('/dns4/bootstrap.paratii.video/tcp/443/wss/ipfs/QmeUmy6UtuEs91TH6bKnfuU1Yvp63CkZJWm624MjBEBazW', (err, success) => {
      if (err) throw err
